# -*- coding: utf-8 -*-
"""
Created on Fri Feb 16 18:49:32 2024
...
first = np.arange(0,21,1)
second = np.arange(22,42,2)
third = np.arange(45,65,5)
fourth = np.arange(70,90,10)
fifth = np.arange(100,120,20)

binlist = np.concatenate((first, second, third, fourth, fifth))
...
###########################################################################
def bin_vis(vis=None, uvdis=None, binlist=None, weight=None):
#Returns the mean visibility and mean uvdistance with respect to the user specified bins
    ...
    for i in range(len(binlist)-1):
        x = np.where(np.logical_and(uvdist > binlist[i], uvdist < binlist[i+1]))      # Indices of vis at correct uv dist
...
a,b,c,d,e = bin_vis(vis=vreal,uvdis=uvdist,binlist=binlist,weight=weights)
f,g,h,k,l = bin_vis(vis=vimag,uvdis=uvdist,binlist=binlist,weight=weights)
...
outfile = open(vis+'_binned_varbinwidth_klambda.txt' ,'w')
